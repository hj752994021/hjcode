...
public class GameSurfaceViewback extends SurfaceView implements Callback, Runnable {
	...
	public GameSurfaceViewback(Context context) {
		super(context);
	...
	public GameSurfaceViewback(Context context, AttributeSet attrs) {
		super(context, attrs);
		// TODO Auto-generated constructor stub
	}
		...
		this.setOnKeyListener(new OnKeyListener() {
			...
			public boolean onKey(View v, int keyCode, KeyEvent event) {
				if(keyCode==62&&event.getAction()==KeyEvent.ACTION_DOWN){
					...
					if(!thread.isAlive()){
						thread = new Thread(GameSurfaceViewback.this);
						thread.start();
					}
					System.out.println("top="+top+" time="+time);
					time=0;
...
class Barrier2 implements Runnable{
	public Barrier2(int x, int y, float width, float height){
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
